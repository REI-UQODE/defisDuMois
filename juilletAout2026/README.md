...
Le deuxième défi de l'été 2026 est un **éditeur / visualisateur MarkDown**!
...
Le but de ce défi est de programmer un **éditeur ou un visualisateur MarkDown dans le langage de votre choix**.

Le MarkDown est un langage de mise en page permettant de créer des beaux documents formatés très facilement avec une syntaxe simple, intuitive et facile à lire en *plain text*. De nos jours, la grande majorité des fichiers README, notamment sur Github, sont écris en MarkDown (incluant celui-ci!).

Votre programme doit permettre de visualiser des fichiers MarkDown (.md), mais l'édition est facultative. Vous pouvez créer un simple visualisateur ou un éditeur et visualisateur en temps différé ou réel.

Les fonctionnalités que votre programme doit obligatoirement supporter sont les suivantes:
- formatage des titres (de `# titre 1` à `###### titre 6`)
- affichage des liens (`[texte] (lien)`s)
- gras et italique (`*italique*`, `**gras**` et `***gras italique***`)
- texte monospace (inline avec des backticks (`), sur plusieurs lignes avec 3 bacticks ou sur une ligne avec une indentation (tab)) (*Vous n'avez pas à supporter la coloration syntaxique*)
- listes multi-niveaux (avec - et *, indenté avec des tabs ou des espaces (selon votre choix))
- listes numérotées

Toutes les autres fonctionnalités du *GitHub flavoured MarkDown* peuvent également être implémentées, selon vos choix.

Vous pouvez vous inspirer des nombreux éditeurs et plugins MarkDown qui sont disponibles en ligne, mais assurez-vous de soumettre le vôtre, avec votre petite touche personnelle.
...
- Fournissez une documentation claire et précise pour exécuter et/ou compiler votre programme. 
    - **TESTEZ CES INSTRUCTIONS AVEC UN COLLÈGUE ET UNE MACHINE (virtuelle) VIERGE AVANT DE SOUMETTRE.** 
    - Si les instructions ne fonctionnent pas, nous vous contacterons et nous vous donnerons une seule chance supplémentaire. Nous ne déboguerons pas votre programme.
- Les instructions doivent permettre d'exécuter votre programme sur une machine Windows et GNU-Linux.
- Pour les soumissions par une plateforme git, nous prendrons la branche principale et le commit le plus récent avant la date limite comme étant la soumission. Toute modification après la date limite sera ignorée.
